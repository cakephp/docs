...
A FormHelper's values sources sets where its rendered elements, such as input-tags, take their values from once the
...
You can this behavior by specifying `sourcesValues` either as an option or by calling it as a setter. The default is
'context'.

The `sourcesValues` can be set to `context`, `data` and `query`. You can either set the Form to use one source
or multiple. Any input fields generated by FormHelper will pick gather its values from the sources, prioritized by the
order set. Examples:

    <?= $this->Form->create($articles, ['valuesSources' => ['query, context']]) ?>
    // or:
    <?= $this->Form->setValuesSources(['query, context'])->create($articles) ?>

Setting custom value sources you can easily create `HTTP GET` based forms, for instance filter forms. Example:

    <?= $this->Form->setSourcesValues('query')->create() ?>
    // ..
    <?= $this->Form->submit() ?>
    <?= $this->Form->end() ?>
