...
特定のプロパティの値を含む要素のリストを構築したい場合は、 ``extract()`` メソッドを
使用することができます。 ::
...
最後に、あなたが取得したいプロパティがパスで表現できない場合は、
...
プロパティを抽出する必要があります。これらの例については、パスのキーに ``{*}`` マッチャを
...
最後に、動的にキーと値とグループのパスを構築するために *クロージャ* を使用することができます。
例えば、エンティティや(ORM によって ``Cake/Time`` インスタンスに変換された) 日付で作業する場合、
...
フィルタ関数で真偽のテストを行うことができます。コレクション内のすべての要素が条件を
...
指定したプロパティを持つ要素のみを含んだ新しいコレクションを抽出する場合、
``match()`` メソッドを使用しましょう。 ::
...
プロパティ名は、ドット区切りのパスになります。ネストされたエンティティを横断し、
...
プロパティに基づいて、コレクションの最小値を抽出するには、 ``min()`` 関数を使用します。
これは、コレクションから、見つかったプロパティの最小値だけでなく完全な要素を返します。 ::
...
また、パスまたはコールバック関数を指定することで、比較するプロパティを表現することができます。 ::
...
同様に、 ``max()`` 関数を使用すると、コレクションから最も高いプロパティ値を持つ要素を返します。 ::
...
最後に、 ``sumOf()`` メソッドは、すべての要素のプロパティの合計を返します。 ::
...
コレクションの要素がプロパティに同じ値を持つ場合、キー別にグループ化した
新しいコレクションを作ることができます。 ::
...
例のごとく、動的にグループを生成するために、ネストされたプロパティのドットで区切られたパス
...
グループ化したいプロパティに対して要素が一意であることがわかっている一定のケースがあります。
...
``groupBy()`` 関数と同じように、プロパティパスまたはコールバックを使用することができます。 ::
...
上で見たように、コレクションの値に存在するカラム名またはプロパティ名を渡すことで並べ替えることができます。
また、代わりにドット表記を使用して、プロパティのパスを指定することができます。
...
コレクションのソート順を指定するには、昇順や降順にソートするために、２番目のパラメータに
...
親の識別子プロパティによって子がグループ化されるような、ネストされた構造を作成するには、
``nest()`` メソッドが簡単です。

この関数には、２つのパラメータが必要です。
１つ目は、項目の識別子を表すプロパティです。
２つ目のパラメータは、親項目の識別子を表すプロパティの名前です。 ::
...
子要素は、コレクション内の各項目の ``children`` プロパティの中にネストされています。
...
戻すように平坦にすることができます。このメソッドは、2つのパラメータを持ちます。
1つ目は、走査モード（昇順、降順または、そのまま）であり、
2つ目は、コレクション内の各要素の子を含むプロパティ名です。
...
あるコレクションの各要素を別のコレクションの各要素のプロパティに挿入することができる
``insert()`` メソッドを提供します。 ::
...
``insert()`` メソッドの最初のパラメータは、要素がその位置に挿入することができるように示した
プロパティのドット区切りのパスです。第２引数は、コレクションオブジェクトに変換することができるものです。

要素が順番に挿入されていることを確認してください。第２のコレクションの最初の要素は、
第１のコレクションの最初の要素にマージされます。

第１のコレクションに挿入する第２のコレクションに十分な要素が存在しない場合、
対象のプロパティは、 ``null`` 値が入力されます。 ::
...
``insert()`` メソッドは、配列の要素や ``ArrayAccess`` インタフェースを実装するオブジェクトを
操作することができます。

コレクションメソッドの再利用
-----------------------------

コレクションのメソッドにクロージャを使用することは、
なすべき仕事が小さくて目的に合うと素晴らしいのですが、とてもすぐに厄介な事になります。
異なる多くのメソッドの呼び出しが必要だったり、クロージャメソッドの長さが数行では収まらないときに、
より顕著になります。

コレクションのメソッドで使用されるロジックは、アプリケーションの複数の部分で再利用できる場合もあります。
複雑なコレクションのロジックを抽出してクラスに分離することを検討してください。
例えば、このような長いクロージャを想像してください。 ::
...
コレクションは不変であり、遅延抽出操作は両方のフィルタのために行われることになるためです。
