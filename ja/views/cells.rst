...
ビューセルはビュー処理やテンプレート出力を実行できる小さな小さなコントローラです。
...
未読メッセージの数を表示したいとします。これはまさにセルを使用する場合です。
私たちが作ったばかりのクラスに、以下を追加します。 ::
...
セルは ``ModelAwareTrait`` と ``ViewVarsTrait`` を使用しているため、コントローラに
とてもよく似たふるまいをします。コントローラ中とまったく同じように、 ``loadModel()``
...
.. note::
    ...
    セルのテンプレートは分離したスコープを持っていて、現在のコントローラのアクション
...
もしもリクエスト中でどのセルを呼び出すかを決定するためのコントローラ処理が必要な場合、
コントローラ中で ``cell()`` メソッドを有効にするために ``CellTrait``
を使用することができます。 ::
...
セルをより柔軟にするために、パラメータ付きのセルのメソッドが必要になることも多いでしょう。
添字付きの配列として ``cell()`` の第二、第三引数を使用することで、アクションのパラメータや、
...
これはアクション名を小文字にしてアンダースコア区切りにしたものに一致するテンプレートを
...
キーが生成される場合には、そのクラスとテンプレートの名前をアンダースコア区切りにしたものが
使用されます。
