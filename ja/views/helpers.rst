...
また、コントローラの ``beforeRender`` メソッドを使用して、ヘルパーを読み込むことができます。 ::
...
次に示すように、コントローラのヘルパーを宣言するときにオプションを指定することができます。 ::
...
デフォルトでは、すべての設定オプションは、 ``$_defaultConfig`` プロパティとマージされます。
このプロパティは、ヘルパーが必要とする設定のデフォルト値を定義する必要があります。例えば::
...
ヘルパーのコンストラクタに提供される任意の設定は、構築時にデフォルト値とマージされ、
...
ヘルパー設定を使用すると、宣言的にヘルパーを設定し、コントローラロジックから設定ロジックを
削除することができます。クラス宣言の一部として組み込むことができない設定オプションがある場合は、
コントローラの beforeRender コールバックで設定します。 ::
...
コントローラで使用するヘルパーを設定すると、各ヘルパーはビューのパブリックプロパティとして公開されます。
...
HelperRegistry は :doc:`レジストリ </core-libraries/registry-objects>` です。
そして、 CakePHP の他の場所で使用されるレジストリ API をサポートしています。
...
フォーマットは、コントローラで指定する場合と同じようにして下さい。 ::
...
ヘルパーが読み込まれたら、一致するビュープロパティにアクセスしてビュー内で使用できます。 ::
