...
- ``cakephp/app`` リポジトリのデフォルトルートクラスは ``DashedRoute`` に変更されました。
...
これらをコントローラで設定する代わりに、これらと同名のメソッドを使ってビューにセットする必要が
あります。 ::
...
これらのメソッドは、コントローラやアクションによって使用されるビュークラスを決定した後に
呼び出されるべきです。

AuthComponent
-------------

- 新しい設定オプション ``storage`` が追加されました。 ``AuthComponent`` がユーザの
...
- ログイン後にリダイレクトする URL を取得するために使用される ``Auth.redirect`` セッション変数を
  ...
  URL を返します。通常の状況下では、ユーザが直接ログインページにアクセスする場合、
...
できるようになりました。この機能は、結果セットやエンティティまたはエンティティのコレクションの中で
...
- ``Query::select()`` は、パラメータとして ``Table`` と ``Association`` オブジェクトを
  サポートします。これらのパラメータの型は、提供されたテーブルまたは関連インスタンスの
