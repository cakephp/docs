...
* ``Cake\ORM\TableRegistry`` の static な API になります。代わりにテーブルロケーターを
...
コードの更新は段階的な処理になる可能性があります。
新しいメソッドの採用に取り組んでいる間、非推奨警告を無効にすることができます。
...
* ページネーションでは、すべてのページの複数のフィールドを並べ替えることができます。
  ...
  さらに、クエリー文字列で定義されたソート条件は、既定の順序を完全に置き換えるのではなく、
  既定の順序パラメーターの先頭に *追加* されます。
* タスククラスが見つからない場合、シェルクラスは例外をスローするようになりました。
  以前は無効なタスクは暗黙のうちに無視されました。
* 可能であれば、CakePHP 内部で例外を連鎖させ、根本的なエラーの原因を露呈させることができます。
* MySQL コネクションのエンコードは ``utf8mb4`` (4バイトユニコード) がデフォルトです。
...
* ``Cake\Database\Type\BatchCastingInterface`` が追加されました。
  このインタフェイスにより、型クラスは、すべての列を一括して操作することによって
...
* ``Validator::addNested()`` と ``addNestedMany()`` は、他のバリデーターメソッドのような
