...
セッションの設定は ``Configure`` の ``Session`` キー下に格納されます。
...
* ``Session.defaults`` - セッションの設定の基礎としてビルトインのデフォルト設定を使用出来ます。
  ビルドインの defaults に関しては、以下をご覧ください。
...
デフォルトでは、 PHP は ``Session.timeout`` の設定に関わらず、ブラウザを閉じると
...
``Session.timeout`` は、ユーザが無活動の合計時間 (例えば、セッションが使われるページを
...
* ``cake`` - セッションをファイルとして ``app/tmp/sessions`` に保存します。ホストの設定が
  ホームディレクトリ以外の書き込みを禁止している場合、有効なオプションです。
* ``database``  ビルトインのデータベースセッションを使用します。詳しくは下記をご覧下さい。
* ``cache`` - ビルトインのキャッシュセッションを使用します。詳しくは下記をご覧下さい。

受け入れられる値は、

* defaults - 上記で説明したように 'php', 'database', 'cache' または 'cake' のいずれか
* handler - ハンドラ設定を含む配列
* ini - セッションを開始する前にセットする php.ini ディレクティブのリスト
* timeout - セッションが有効な期間 (分)
...
セッションの破棄は、セッション内の全てのサーバ側データを削除しますが、セッションクッキーの
**削除はしません** 。
