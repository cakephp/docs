...
プレフィックスを利用して各コンテキストないで異なるコントローラを使用できます。 ::
...
マップします。コントローラを分けることで、あなたのコントローラのロジックをシンプルに保つことが
...
リダイレクトルートは通常のルートとは異なり、条件に一致した場合、現在のヘッダーにリダイレクトを
実行します。リダイレクトは、アプリケーション内部や外部へ遷移します。 ::
