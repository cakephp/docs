...
CakePHP の Router はリバースルーティングできます。それは、一致するパラメーターを含む配列から、
...
``connect()`` の残り２つのパラメーターを使います。２番目のパラメーターは、連想配列です。この配列のキーは、
URL テンプレートが示すルート要素に因んで命名すべきです。配列の値はキーのためのデフォルトの
値になります。 ``connect()`` の３番目のパラメーターを使う前に基本的な例を見てみましょう。 ::
...
CakePHP 2.x アプリケーションから移行するときに、小文字とアンダースコアによる
...
* ``_base`` 生成された URL からベースパスを削除するには ``false`` をセットしてください。
  アプリケーションがルートディレクトリにない場合、 'cake relative' な URL の生成に使えます。
...
デフォルトで、複数語のコントローラーの URL フラグメントは、コントローラー名のアンダースコア形式です。
...
* ``_base`` 生成された URL からベースパスを削除するには ``false`` をセットしてください。
  アプリケーションがルートディレクトリにない場合、 'cake relative' な URL の生成に使います。
* ``_scheme`` ``webcal`` や ``ftp`` のように異なるスキーマのリンクを作成するために設定します。
...
URL を文字列で生成します。URL パラメーターがルートに一尉しない時は、 ``false`` を返します。
