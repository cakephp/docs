...
例えば、記事は多くのコメントを持っていて書き手に属しています。
書き手は多くの記事とコメントを持っています。 CakePHP はこうしたアソシエーションの管理を
...
------------- --------------------- -----------------------------------------
他 対 １      belongsTo             多くの記事がユーザに属している。
------------- --------------------- -----------------------------------------
他 対 多      belongsToMany         タグは多くの記事に属している。
============= ===================== =========================================
...
複数のアソシエーションを受け取ることができます。もしも数値キーが使用された場合は
...
このアソシエーションが定義された後は、 Users テーブルの検索操作で、もしも Address
...
このアソシエーションが定義された後は、 Addresses テーブルの検索操作で、もしも User
のレコードが存在すればそれを含むことができます。 ::
...
このアソシエーションが定義された後は、 Articles テーブルの検索操作で、もしも Comment
のレコードが存在すればそれを含むことができます。 ::
...
持たせないようにすべきです。もしも例えば、アソシエーションのプロパティ名と衝突する
...
- **joinTable**: このアソシエーションで使われる結合テーブルの名前
  （現在のテーブルがbelongsToMany結合テーブルの命名規則に準拠していない場合）。
...
このアソシエーションが定義された後は、 Articles テーブルの検索操作で、もしも Tag
のレコードが存在すればそれを含むことができます。 ::
...
もしも結合テーブルに追加の情報を持たせようとしている場合、あるいはもしも規約から外れる
