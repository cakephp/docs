...
.. note::
    ...
    デフォルトでは、CakePHP は、セキュリティ上の理由のためにフラッシュメッセージ中の内容は
    エスケープします。もし、任意のリクエストやユーザーデータをフラッシュメッセージ中で使用する
    場合は、これらはエスケープされるため安全に表示されます。もし、HTML を出力したい場合、
    ``escape`` パラメータをセットする必要があり、そのパラメータがセットされた際に、
    エスケープを無効にするようにテンプレートを調整してください。
