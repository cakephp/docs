...
アプリケーションの **composer.json** ファイルに下記のように追記する必要があります。
...
* ``console`` アプリケーションのコマンドコレクションにコンソールコマンドを
  追加するために使用されます。
* ``events`` イベントリスナーをアプリケーションのイベントマネージャに追加するために使用されます。

プラグインをロードするとき、有効にするフックを設定できます。
デフォルトでは、a:ref:`plugin-objects` のないプラグインはすべてのフックを無効にします。
新しいスタイルのプラグインを使用すると、プラグイン作成者はデフォルトを設定できます。
これは、アプリケーションによって設定されます。 ::
...
例えば、あなたの画面で素敵な連絡先情報を表示するために、 ContactManager プラグインの
...
プラグインは、 ``console()`` フックの中に、コマンドを登録することができます。
