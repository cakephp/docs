...
コントローラは、タスクを終える為の全ての必要とされるリソースが正しい労働者に委譲されることに注意を払う
マネージャーと見ることができます。
...
#. ウェブサーバが **webroot/index.php** へのリクエストを制御するルールを書き換えます。
...
#. ビューがヘルパーとセルを使ってボディとヘッダを生成して返す。
#. レスポンスは、 :doc:`/controllers/middleware` を経由して送信されます。
#. ``HttpServer`` は、ウェブサーバにレスポンスを送ります。
