...
モデルを作成するには、記事のコントローラーが必要です。CakePHP のコントローラーは、
HTTP リクエストを処理し、モデルのメソッドに含まれるビジネスロジックを実行し、
レスポンスを準備します。この新しいコントローラーは **src/Controller** ディレクトリーの中で
...
それでは、コントローラーにアクションを追加しましょう。アクションは、ルートに接続された
...
にしたがって、読みやすく意味のあるアクション名を作成してください。次に
...
データベースからページ付けされた記事を取得します。次に (これからすぐに作成する)
...
基本的な読み込みビューを作成すると、新しい記事を作成できるようにする必要があります。
...
* なんらかの理由で保存しなかった場合、ビューを描画します。これは、ユーザーバリデーションエラーやその他の
  警告を表示する機会を与えてくれます。
...
データを保存するために、まず Article エンティティーの中に POST データを「マーシャリング」します。
...
フラッシュメッセージは、リダイレクトした後の次のページ上で表示されます。
...
FormHelper を使うと、 HTML フォームのオープンタグを生成できます。
...
ここで使用されるイントロスペクションと規約が少しあります。 ``control()`` は、指定された
モデルフィールドにもとづいて異なるフォーム要素を出力し、語形変化 (inflection) を使って
ラベルを生成します。オプションを使用して、フォームコントロールのラベル、入力、または
その他の要素をカスタマイズすることができます。 ``$this->Form->end()`` の呼び出しで
フォームを閉じます。
...
次に、アクションは POST リクエストか PUT リクエストかをチェックします。
...
検証ルールが整ったら、アプリを使って空のタイトルや本文を追加して記事の仕組みを見てみましょう。
...
このアクションにはテンプレートはありません。ユーザーが記事を削除できるリンクが付けるように
index テンプレートを更新することができます。
