...
もしくは ``composer.phar`` を`Composer website <https://getcomposer.org/download/>`_から落として下さい.
...
.. note::
    ...
    Windows　では、このコマンドは ``bin\cake`` (バックスラッシュではない)です。.

ポート8765でビルトインのPHPサーバーは動きます。ポートを開いて下さい
**http://localhost:8765** でウェブサーバーのwelcom画面が開くか確かめて下さい。
...
なぜなら、毎回パスワードを暗号化するときにmutator/setter　メソッドを使うからです。
 Cake　は規約ベースのセッターメソッドをエンティーにセットします。
...
表示される。CakePHP `bcrypt
<http://codahale.com/how-to-safely-store-a-password/>`　でハッシュ化するのがデフォルト設定です。
...
リクエストデータの他の部分にアクセスするためには:`cake-request`
を見てください。
...
<http://php.net/manual/en/functions.anonymous.php>`_を受け付けます。 これは、クエリ日ヅダーの引数を受け付けます。
...
``tags.ctp`` CakePHPの規約に従ってビューテンプレートファイルから作られます。
この規約は
小文字を使って、'_'を利用したアクション名と同じ名前にする必要があります。
...
You should now be able to visit the **/bookmarks/tagged/funny** にアクセスすると、
