...
.. note::
    ...
    これは、DELIMITER をサポートしていないストアドプロシージャまたはトリガの挿入時に
...
``fetchAll()`` メソッドは複数の行を返します。どちらのメソッドも、唯一のパラメータとして
生の SQL を受け取ります。 ::
...
scale      ``precision`` と組み合わせ、少数精度を設定
...
インデックスを削除するのは、 ``removeIndex()`` メソッドを呼び出すのと同じくらい簡単です。
