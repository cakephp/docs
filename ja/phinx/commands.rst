...
特定のバージョンにマイグレーションするには、 ``--target`` パラメータまたは省略して
``-t`` を使用します。
...
すべてのマイグレーションを特定のバージョンにロールバックするには、 ``--target`` パラメータまたは
省略して ``-t`` を使用します。
...
すべてのマイグレーションを特定の日付にロールバックするには、 ``--date`` パラメータまたは省略して
``-d`` を省略して使用します。
...
ブレークポイントが設定され、さらにロールバックをブロックしている場合は、 ``--force`` パラメータまたは
...
メモリデータベースを使用する場合は、Phinx に特定の PDO インスタンスを提供する必要があります。
