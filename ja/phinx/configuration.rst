...
Phinx はプロジェクトディレクトリのルートに ``phinx.yml`` というデフォルトファイルを作成します。
...
最初のオプションは、マイグレーションのディレクトリへのパスを指定します。
デフォルトでは Phinx は ``%%PHINX_CONFIG_DIR%%/db/migrations`` を使用します。

.. note::

    ``%%PHINX_CONFIG_DIR%%`` は特別なトークンで、
    ``phinx.yml`` ファイルが保存されているルートディレクトリに自動的に置き換えられます。
...
マイグレーションは ``glob`` で取り込まれるので、複数のディレクトリのパターンを定義することができます。
...
2番目のオプションは、シードディレクトリへのパスを指定します。
