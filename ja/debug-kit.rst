...
ルートディレクトリ (composer.json ファイルのある場所) で実行してください。 ::
...
DebugKit ツールバーはいくつかのパネルが含まれており、ブラウザウィンドウの右下の
...
パネルクラスは単に **src/Panel** ディレクトリに設置してください。ファイル名はクラス名と
...
にプラグインディレクトリの名前を必ずセットする必要があります。 ::
