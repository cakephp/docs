...
ユーザがダウンロード可能な PDF の生成もビューレイヤーの役割となります。
...
下記の各制御構造はこうした閉じ構文を持っています。
``endif`` 、 ``endfor`` 、 ``endforeach`` 、そして ``endwhile`` です。
各制御構造の後（最後の一つを除いて）に ``セミコロン (;)`` ではなく、
``コロン (:)`` があることにも注意してください。
...
テンプレートファイルは **/src/Template/** の中の、ファイルを使用するコントローラにちなんで
名付けられたフォルダに置かれ、その対応するアクション名にちなんで名付けられます。
例えば、 Products コントローラの "view()" アクションのビューファイルは通常、
**/src/Template/Products/view.ctp** となります。

CakePHP のビューレイヤーはいくつかの異なるパーツによって作り上げられています。
各パーツは異なる役割を持っており、この章で説明していきます。

- **ビュー**: テンプレートは実行中のアクション固有のページの一部分です。
  アプリケーションの応答の中心となります。
...
ビューにはコントローラオブジェクトの ``set()`` と類似の ``set()`` メソッドがあります。
...
時にはコントローラの中で、ビュー変数としてタイトルを代入するかもしれません。 ::
...
.. note::
    ...
    ``content`` をブロック名として使うことは避けるべきです。これは継承されたビュー、
    あるいはレイアウト中のビューコンテンツのために CakePHP によって内部的に使われます。
...
これによって、ブロックが存在しない場合のプレースホルダを追加することでききます。
既定値は第二引数で指定します。
...
なぜならページが描画される時には、コントローラによって描画されるビューのコードは、
既定のレイアウトの中に置かれるからです。
...
好きなだけレイアウトを作ることができます。 **src/Template/Layout** ディレクトリに置いて、
コントローラのアクションの中か、ビューの ``$layout`` プロパティで切り替えるだけです。 ::
...
エレメントは  **src/Template/Element/** フォルダの中に .ctp の拡張子を持つ名前で配置されます。
...
パラメータ配列のメンバとして利用できます。(テンプレートファイルにおけるコントローラの
...
毎回異なる名前の 'key' パラメータを設定するようにしてください。
...
プラグインのコントローラ/アクションで描画される場合、他のプラグイン名が渡されなければ、
使用されるすべてのエレメントに対して自動的にそのプラグイン名が接頭されます。
もしエレメントがプラグインに存在しない場合、メインの APP フォルダの中を探します。 ::
...
ルーティングプレフィクスを設定している場合、レイアウトやビューがそうであるように、
エレメントのパスの解決は、プレフィクスの場所に切り替わります。
"Admin" プレフィクスが設定されていて、次のように呼ぶとすれば::
...
コントローラと同様、ビューは、描画のライフサイクル回りのロジックを入れるために、
