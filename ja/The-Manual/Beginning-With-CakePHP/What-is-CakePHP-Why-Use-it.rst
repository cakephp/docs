...
CakePHPにはアクティブな\ `開発チーム <http://cakephp.lighthouseapp.com/contributors>`_\ と\ `コミュニティ <http://cakephp.org/feeds>`_\ が存在し、プロジェクトに貢献しています。CakePHPを使用する、ということは、車輪の再開発を避けるだけでなく、あなたが開発するアプリケーションのコア部分がよくテストされ、常に改良されている、ということになります。
...
-  アプリケーションの\ `scaffolding（足場組み） <http://en.wikipedia.org/wiki/Scaffold_%28programming%29>`_
-  コード生成
-  `MVC <http://en.wikipedia.org/wiki/Model-view-controller>`_
   アーキテクチャ
-  クリーンでカスタマイズ可能なURLsとroutesを用いるリクエストディスパッチャー
-  `バリデーション <http://en.wikipedia.org/wiki/Data_validation>`_\ を内蔵
