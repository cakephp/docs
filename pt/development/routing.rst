...
O roteamento fornece ferramentas que mapeiam URLs para actions de controller. Ao definir
rotas, você pode separar como sua aplicação é implementada de como suas URLs
são estruturadas.

O roteamento no CakePHP também engloba a ideia de roteamento reverso, onde um array
de parâmetros pode ser transformado em uma string URL. Ao usar o roteamento reverso,
você pode refatorar a estrutura de URL da sua aplicação sem ter que atualizar todo
o seu código.
...
Esta seção irá ensinar você por exemplos os usos mais comuns do
Router do CakePHP. Normalmente você quer exibir algo como uma página inicial, então você adiciona
isto ao seu arquivo **config/routes.php**::
...
To generate asset URLs for files in plugins use :term:`sintaxe de plugin`::
...
standard :term:`sintaxe de plugin`.
