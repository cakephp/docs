...
Desde o começo, o suporte a javascript no CakePHP era para o
Prototype/Scriptaculous. Por mais que nós ainda achemos que essas são
duas excelentes bibliotecas javascript, a comunidade já vinha
solicitando suporte para outras bibliotecas. Ao invés de remover o
Prototype, substituindo-o por alguma outra biblioteca de javascript, nós
criamos um helper baseado no padrão adapter e incluímos suporte para as
três bibliotecas mais requisitadas. Prototype/Scriptaculous,
Mootools/Mootools-more e jQuery/jQuery UI. E ainda que a API não seja
tão expansiva quanto permitia o AjaxHelper anterior, nós sentimos que a
solução baseada no padrão adapter é a solução mais extensível e que dá
aos desenvolvedores o poder e a flexibilidade de que eles precisam para
aplicar às necessidades específicas de suas aplicações.

As bibliotecas de javascript formam a espinha dorsal do novo JsHelper.
Uma biblioteca de javascript traduz um elemento javascript abstrato em
código javascript concreto e específico para a biblioteca de javascript
que estiver sendo utilizada. Além disso, elas criam um sistema
extensível para ser utilizado por outros.

Usando uma biblioteca de Javascript específica
==============================================

Antes de mais nada, faça o download de sua biblioteca javascript e
coloque-a em ``app/webroot/js``

Agora você deve incluir a biblioteca na sua página. Para fazer isto em
todas as páginas, inclua esta linha na seção <head> de seu
``app/views/layouts/default.ctp`` (se você não tiver seu próprio arquivo
de layout, copie este arquivo de
``cake/libs/view/layouts/default.ctp``).

::

    echo $this->Html->script('jquery'); // inclui a biblioteca jQuery

Substitua ``jquery`` pelo nome do arquivo de sua biblioteca em questão
(a extensão .js será adicionada ao nome).

Por padrão, os scripts são guardados em cache e você deve exibir o
conteúdo do cache explicitamente. Para fazer isto ao final de cada
página, inclua esta linha logo antes da tag de fechamento ``</body>``
tag

::

    echo $js->writeBuffer(); // Escreve o conteúdo em cache dos scripts

A seleção de qua ferramenta de javascript usar é declarada no momento em
que você inclui o helper no seu controller.

::

    var $helpers = array('Js' => array('Jquery'));

Este trecho de código acima irá utilizar o Jquery nas instâncias do
JsHelper em suas views. Se você não declarar nenhuma ferramenta de
javascript, jQuery será a biblioteca utilizada por padrão. Como
mencionado anteriormente, são três as ferramentas de javascript
suportadas por padrão, mas encorajamos a comunidade a expandir o suporte
ainda a outras bibliotecas.
...
::
    ...
    <a href="/pt/posts/index/page:2" other="value">Page 2</a>
