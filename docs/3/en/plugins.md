...
plugins without a [Plugin Objects](tmp.MdTas7IsIA#plugin-objects) have all hooks disabled. New style plugins
...
See the section on [Plugin Configuration](tmp.MdTas7IsIA#plugin-configuration) for information on how to load
...
done as per the documentation [Autoloading Plugin Classes](tmp.MdTas7IsIA#autoloading-plugin-classes).
