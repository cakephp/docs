...
.. php:method:: sort($key, $title = null, $options = [])
    ...
    :param string $title: Заголовок для ссылки. Если $title имеет значение null, $key будет использоваться
    для преобразования в формат "Title Case" и как заголовок.
...
* ``last`` Если вы хотите сгенерировать ссылки на последние страницы, установите целое число для определения 
числа 'последних' ссылок для генерации. По умолчанию используется ``false``. Следуйте той же 
логике, как и для опции ``first``. Или, если хотите, используйте :php:meth:`~PaginatorHelper::last()`.
...
.. php:method:: prev($title = '<< Previous', $options = [])
    ...
    * ``disabledTitle`` Текст для использования, когда ссылка отключена. По умолчанию используется 
    параметр ``$title``.
...
.. php:method:: next($title = 'Next >>', $options = [])
    ...
    Он создаёт ссылки, указывающие на следующую страницу, а не на предыдущую. Это также
    использует ``next`` в качестве значения атрибута rel вместо ``prev``.
...
.. php:method:: first($first = '<< first', $options = [])
    ...
    - ``escape`` Следует ли экранировать текст. Установите значение ``false``, если ваш
    контент содержит HTML.
...
Вывод, выданный методом counter(), также можно настроить с помощью специальных
маркеров::
