...
Помощники(далее по тексту хелперы (Helpers))
это классы похожие на Коспоненты, служащие для слоя отображения. Хелперы 
содержат логику отображения, и могут быть использованы в Видах(Views), 
Элементах(Elemwnts) и Шаблонах(Layouts). В данной главе показано как
создать свой Хелпер, а так же немечены основные задачи которые помогут
решить Хелперы CakePHP

CakePHP имеет ряд Хелперов, которые помогают в создании представления.
Они помогают в создании хорошо сформированной разметки (в том числе форм), помогают
ри форматировании текста, дат и времени, цифр, и даже может ускорить функциональность AJAX.
Для получения дополнительной информации о Хелперах, включенных в CakePHP,
посмотрите раздел для каждого Хелпера:
