...
Когда пользователь отправляет данные через форму методом POST, эта информация становится
доступна в свойстве ``$this->request->data``. Вы можете использовать функцию
:php:func:`pr()` или :php:func:`debug()` для распечатки содержимого этого свойства.
