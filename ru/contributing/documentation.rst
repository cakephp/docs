...
`непрерывно интегрируемая <https://en.wikipedia.org/wiki/Continuous_integration>`_,
...
- Используйте `Неофициальные формы <https://en.wikipedia.org/wiki/Register_(linguistics)>`_.
...
- Не используйте
  `HTML сущности <https://en.wikipedia.org/wiki/List_of_XML_and_HTML_character_entity_references>`_
...
`Текста в формате ReST <https://en.wikipedia.org/wiki/ReStructuredText>`_. ReST
...
Полученная ссылка будет выглядеть так: `External Link to php.net <https://php.net>`_
...
<https://pypi.python.org/pypi/sphinxcontrib-phpdomain>`_ для предоставления пользовательских
