...
#. 데이터베이스 서버.
    ...
    데이터베이스를 만들 수있을 정도의 SQL 지식이 필요합니다. 그 앞은 CakePHP가 돌
    본줍니다. MySQL을 사용하기 때문에 PHP에서 ``pdo_mysql`` 가 활성화되어 있는지
    확인하시기 바랍니다.
...
어떤 이유로 CakePHP가 그 디렉토리에 쓸 수없는 경우 캐시 된 데이터가
...
'created' 와 같은 필드가있을 때 자동적으로 CakePHP가 관리되는 것처럼 보입니다.
