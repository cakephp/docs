...
o de la extensión del enrutamiento. Si tu aplicación necesita realizar una lógica
diferente para diferente formatos de respuesta puedes usar ``$this->request->is()``
para construir la lógica condicional requerida.
...
.. versionadded:: 4.4.0
    Anterior a 4.4 debes usar :doc:`/controllers/components/request-handling`
    en vez de ``viewClasses()``.
...
La función ``fetchTable()`` es útil cuando se necesita usar una tabla que no es 
